...
targetFolder =  '/home/hakhan/Google Drive/p2pCarRentalProject/AnalysisScripts/IntermediateData/reviewerImages/APPNAME/'
...
passedCity = int(sys.argv[1])
cities = [cities[passedCity]]
# print(cities)
# time.sleep(10000)

for city in cities:
    
    for platform in ['GerAroundEurope']:
        ...
        for item in filenames:
            cityName = item.split('~')[0]
            if cityName == city:
                filenamesNew.append(item)
        filenames = filenamesNew
        if len(filenames) > 0:
            print('\t', platform, city, len(filenames))
            # time.sleep(10000)
            start = timer()
            ...
            savePath = '/home/hakhan/Google Drive/p2pCarRentalProject/AnalysisScripts/IntermediateData/4j2-output/'
            fx = open(savePath+platform+'~'+city+'-demographicsReviewer.txt','w')
