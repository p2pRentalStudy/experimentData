            ...
            # try:
                ...
                for i in range(0, len(totalDates)):
                    ...
                    for vehicleId in content[keyDate]:
                        ...
                        try:
                            val = vehiclePrices[vehicleId]
                            # print(keyDate, vehicleId, vehiclePrices)
                            # time.sleep(1000)
                        except:
                            vehiclePrices[vehicleId] = {}
                            vehiclePrices[vehicleId]['data'] = []
                            vehiclePrices[vehicleId]['constantVars'] = []
                            
                        vehiclePrices[vehicleId]['data'].append((bookingPrice, keyDate, car['carRankInSearch']))
                        try:
                            vehiclePrices[vehicleId]['constantVars'] = [(car['trips'], car['averageRating'], car['ownerID'])]
                            # print(vehiclePrices[vehicleId]['constantVars'])
                        except:
                            pass

            savePath = '/home/hakhan/Google Drive/p2pCarRentalProject/AnalysisScripts/IntermediateData/15-output/'+platform+'/'         
            fx = open(savePath + platform+'-'+cityName+'.txt','w')
            fx.write(json.dumps(vehiclePrices))
            fx.close()
