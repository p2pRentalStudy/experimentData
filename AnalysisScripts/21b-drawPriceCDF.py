...
fx = open(savePath+'21a-tripPriceDict.txt','r')
...
for platform in platformColor:
    ...
    try:
        ...
        for city in totalVehiclesPrice[platform]:
            data += totalVehiclesPrice[platform][city]
            try:
                if len(totalVehiclesPrice[platform][city]) > 0:
                    print('\t',city, np.average(totalVehiclesPrice[platform][city]))
                else:
                    print('\t',city, 'no data')
            except:
                print('\t',city, 'no data')
...
plt.xlim([0,500])
plt.title('hourly price per trip')
