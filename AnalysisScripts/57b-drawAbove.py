...
for item in tbsData:
    # if item[0]< '50':
    #     item[1] = item[1]* 0.5
    # if item[0]> '30' and  item[0] <'150':
    #     item[1] = item[1]* 100.5

    # if item[0] > '190':
    #     item[1] = item[1]* 0.7

    # barData.append(item[1]*1.3)
    barData.append(item[1])
    
    totalData.append(['price '+item[0], round(barData[-1],2)])
    if ind % 30 == 0:
        possibleColors.append(item[0])
        xlabels.append(ind)
    # if ind == len(tbsData)-1:
    #     # possibleColors.append('550')
    #     xlabels.append(ind)
...
X_Y_Spline = make_interp_spline(x_pos, barData)
 
# Returns evenly spaced numbers
# over a specified interval.
# X_ = np.linspace(x_pos.min(), x_pos.max(), 200)
# Y_ = X_Y_Spline(X_)
 
# cubic_interpolation_model = interp1d(x_pos, barData, kind = "cubic")

# ax.plot(X_, Y_, alpha=0.5)
from scipy.ndimage.filters import gaussian_filter1d
for i in range(0, len(barData)):
    barData[i] = barData[i]*1
barData = gaussian_filter1d(barData, sigma=2)
ax.plot(x_pos, barData, alpha=0.5, lw=3)
# Plotting the Graph
...
plt.xticks([20,35, 50, 70, 100, 150, 200,300, 500, 850, 1500],[20,35, 50, 70, 100, 150, 200,300, 500, 850, 1500], fontsize=20, rotation=90)
# ax.set_xlabel('Price in U', fontsize=17)
plt.xlim(15,1500)
