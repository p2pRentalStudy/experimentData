...
for platform in totalTripsPerOwnerData:
    ...
    for cityName in totalTripsPerOwnerData[platform]:
        try:
            v10 = cityList[cityName]
        except:
            cityList[cityName] = [{}, {}, {}, {}, {}, {}]
            cityList['max'+cityName] = 0
        for ownerID in totalTripsPerOwnerData[platform][cityName]:
            ...
            cityList[cityName][min(5,ownerCars-1)][ownerID+platform] = 1
            cityList['max'+cityName] = max(cityList['max'+cityName], ownerCars)
            totalOwners += 1
            for carID in totalTripsPerOwnerData[platform][cityName][ownerID].keys():
                totalCars += 1
                
print(totalOwners, totalCars)
# time.sleep(10000)

cities = list(cityList.keys())
cities.sort()
for city in cities:
    totalOwners = 0
    ...
    if 'max' not in city:
        for item in cityList[city]:
            totalOwners += len(item.keys())
        print(cityShort[city], end=' & ')
        for i in range(0,6):
            print(round(len(cityList[city][i].keys())/totalOwners * 100, 1), end=' &')

        print(cityList['max'+city],'\\\\')
        print()
