...
fx = open('averageRatingDict.txt', 'r')
averageRatingDict = json.loads(fx.read())
fx.close()
...
i = 0.0
while i < 5.01:
    makersTypes.append(str(round(i,1)))
    i += 0.1
    ...
    for cityName in analyzedData[platform]:
        if cityName != 'New York':
            ...
            for carId in analyzedData[platform][cityName]:
                ...
                dictRating = -1
                try:
                    dictRating = averageRatingDict[platform][cityName][carId]
                    if len(dictRating) > 0:
                        carRating = np.average(dictRating)
                except:
                    pass

                # print(platform, city,carId, carRating, carRating, dictRating)
                # time.sleep(10000)
                if experimentTripsVehicle > -1 and carRating>=0:
                    carRating = round(float(carRating),1)
                    experimentTripsVehicle = (experimentTripsVehicle/carDays)*30
                    colorVehicleTripsDict[platform][cityName][str(carRating)]['trips'].append(round(experimentTripsVehicle,2))
