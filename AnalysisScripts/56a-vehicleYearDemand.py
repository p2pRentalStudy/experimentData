    ...
    for cityName in analyzedData[platform]:
        if cityName != 'New York':
            ...
            for carId in analyzedData[platform][cityName]:
                ...
                dates.sort()
                carYear = carObj[dates[0]]['year']
                # print(platform, city,carId, carYear, catObj)
                # time.sleep(10000)
                if experimentTripsVehicle > -1:
                    try:
                        experimentTripsVehicle = (experimentTripsVehicle/carDays)*30
                        colorVehicleTripsDict[platform][cityName][str(carYear)]['trips'].append(round(experimentTripsVehicle,2))
                    except:
                        pass
