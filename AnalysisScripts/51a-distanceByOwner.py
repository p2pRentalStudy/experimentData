        ...
        for ownerID in totalTripsPerOwnerData[platform][cityName]:
            ...
            for carID in totalTripsPerOwnerData[platform][cityName][ownerID].keys():
                ...
                for tripObj in totalTripsPerOwnerData[platform][cityName][ownerID][carID]['trips']:
                    # print(tripObj)
                    ownerIncome += tripObj['length']
            # print(ownerIncome, carDays, ownerCars)
            # time.sleep(1)
            totalData[min(5, ownerCars-1)].append(ownerIncome/carDays)
            heatmapData[cityName][min(5, ownerCars-1)].append(ownerIncome/(carDays*ownerCars))
...
sns.heatmap(df, annot=True, annot_kws={"size": 7}, yticklabels= ylabels,  fmt="d")#, vmin=20, vmax=100#, linewidths=2,)

plt.ylabel('Cities', fontsize=12)
plt.xlabel('Vehicles owners by the owner', fontsize=12)

plt.title('Heatmap of the average trip distances across multiplicitiy', fontsize=14)
