            ...
            try:
                ...
                for recDate in carPricingDict[vehicle]:
                    ...
                    for item in pricesList:
                        # print(item, x, recDate, totalPrice,pricesList)
                        # time.sleep(1000)
...
tempKeys = list(cityDayPrices.keys())
tempKeys.sort()
for cityin in range(len(tempKeys)):
    city = tempKeys[cityin]
...
df = pd.DataFrame(heatmapData, columns=list(weekdays))
...
plt.title('Average Booking Price % Observed Across Months', fontsize=13)
