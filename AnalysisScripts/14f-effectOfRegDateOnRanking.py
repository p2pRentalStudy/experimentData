        ...
        if 1:
            ...
            for i in range(0, len(totalDates)):
                ...
                for id in content[date]:
                    ...
                    checkKey = 'listingCreated'
                    try:
                        nextKeys = list(content2[id].keys())

                        bookingPrice = str(content2[id][nextKeys[0]][checkKey])[:10]
                        d2 = datetime.datetime.fromtimestamp( int(bookingPrice) ).date()
                        # print(d2)
                        delta = d1 - d2
                        # print(d2, delta.days)
                        # time.sleep(1000)
                        ratings.append(delta.days)
