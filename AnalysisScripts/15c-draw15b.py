...
fx = open(savePath+'15b-priceChangeDict.txt','r')
...
for platform in platformColor:
    try:
        N = len(totalVehiclesPrice[platform])
...
plt.title('CDF of price changes per vehicles')
...
scriptName = '15c'
fig.savefig('plots/'+scriptName+'-cdfPriceChangeCount.png') 
fig.savefig('plots/'+scriptName+'-cdfPriceChangeCount.eps') 
